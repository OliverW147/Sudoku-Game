% Generation will follow an alg outlined by Andrew C. Stuart in an online
% paper named "Sudoku Creation and Grading"

% Fill 9 random cells with 1 to 9
% Solve the puzzle
% Set cells to zero at random until the puzzle is not solvable

function [generated_matrix] = generate_matrix(difficulty)
    if strcmp(difficulty, "easy")
        generated_matrix = [
    0, 0, 3, 0, 2, 0, 6, 0, 0;
    9, 0, 0, 3, 0, 5, 0, 0, 1;
    0, 0, 1, 8, 0, 6, 4, 0, 0;
    0, 0, 8, 1, 0, 2, 9, 0, 0;
    7, 0, 0, 0, 0, 0, 0, 0, 8;
    0, 0, 6, 7, 0, 8, 2, 0, 0;
    0, 0, 2, 6, 0, 9, 5, 0, 0;
    8, 0, 0, 2, 0, 3, 0, 0, 9;
    0, 0, 5, 0, 1, 0, 3, 0, 0];
    end
    if strcmp(difficulty, "hard")
        generated_matrix = [0,0,5,3,0,0,0,0,0;8,0,0,0,0,0,0,2,0;0,7,0,0,1,0,5,0,0;4,0,0,0,0,5,3,0,0;0,1,0,0,7,0,0,0,6;0,0,3,2,0,0,0,8,0;0,6,0,5,0,0,0,0,9;0,0,4,0,0,0,0,3,0;0,0,0,0,0,9,7,0,0];
    end
end